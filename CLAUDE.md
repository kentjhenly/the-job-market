...
**Revenue:** Employer subscriptions. Employers get a **free trial** before any subscription is required: their first 3 job postings are free, and they can send pitches from those first 3 postings for free. Beyond the trial, an active subscription (`employers.subscription_status = 'active'`, tier `starter`) is required to create more postings and to keep sending pitches. **Browsing the candidate feed (`/employer/feed`) always requires an active subscription** — there is no free-trial allowance for the feed. Accepting pitches is always free for candidates. See **Employer Subscriptions** under Key Conventions.
...
- Subscription columns: `employers.subscription_tier` (`none | starter`), `employers.subscription_status` (`active | past_due | canceled`), `employers.subscription_period_end` (added in `0025_employer_subscriptions.sql`).
- **Browsing `/employer/feed`** always requires `subscription_status === 'active'`. When inactive, the page renders `UpgradePanel` instead of `FeedClient` (there is no free-trial allowance for the feed).
- **Sending a pitch** (`POST /api/matches`) is allowed without an active subscription only during the free trial: an employer who has **never subscribed** (`subscription_tier === 'none'`) can pitch when the pitch's `posting_id` is one of their first `FREE_JOB_POSTINGS` (3) postings (oldest by `created_at`). Otherwise — no active subscription and either no `posting_id`, a posting beyond the first 3, or a `tier` that has since lapsed from a real subscription — it returns `402`. With `subscription_status === 'active'`, pitching is unrestricted.
...
- **Stripe billing:** `UpgradePanel` (`/employer/feed`) offers a single STARTER tier (HKD 150/mo) and POSTs `{ tier: "starter" }` to `POST /api/subscription/checkout`, which creates a Stripe Checkout subscription session (price ID from `STRIPE_PRICE_STARTER`, `employer_id`+`tier` stamped into both session and subscription metadata) and returns the redirect `url`. On return, `GET /api/subscription/success?session_id=...` verifies the session and flips the employer to `active`. Ongoing sync is handled by `POST /api/subscription/webhook` (`customer.subscription.created/updated/deleted`, signature-verified via `STRIPE_WEBHOOK_SECRET`): it maps Stripe's status to `active | past_due | canceled`, sets the tier/period-end from the subscription's metadata + first item's `current_period_end`, all keyed by `employer_id` in the subscription metadata. Env: `STRIPE_SECRET_KEY`, `STRIPE_PRICE_STARTER`, `STRIPE_WEBHOOK_SECRET` (see `.env.example`). The display price in `UpgradePanel` is static UI copy; the actual charge comes from the Stripe price object, so keep the two in step when you change a price in the dashboard. The columns remain manually-settable (e.g. via `/admin/concierge`) as a fallback when Stripe keys aren't configured.
- UI surfaces the current tier: `TopBar` employer PLAN stat, `/employer/terminal`'s SUBSCRIPTION tile (links to `/employer/feed` to upgrade when inactive).
...
| `/candidate/terminal` | Candidate | Score terminal (live score, radar, salary curve) |
...
| `/employer/postings/[postingId]` | Employer | Create/edit posting + ranked matched-candidates panel (pitch directly from a match) + posting lobby (`PostingLobbyClient`): sent pitches + match status with unread indicators + CHAT → in-app chat slide-over for accepted matches. There is no separate `/employer/matches` route; the employer manages matches per-posting here and sees an aggregate view on `/employer/terminal`. |
...
- Redirects employers from candidate routes to `/employer/terminal`
- Redirects candidates from employer routes to `/candidate/terminal`
...
| `candidates` | composite_score, percentile_rank, years_exp_claimed, current_salary (monthly cents), desired_salary_* (legacy), location (legacy), is_visible, is_founder_verified, date_of_birth (legacy, no longer edited/displayed), sex (legacy, no longer edited/displayed), languages, citizenship (biodata + experience + current salary all edited in `/candidate/settings`) |
...
| `matches` | employer_id, candidate_id, posting_id, candidate_posting_id (the candidate posting the pitch targeted, removed on hire), status (pending/accepted/declined/ghosted/withdrawn), offered_salary, expires_at (72hr), offer_status (null/pending/accepted/declined), offer_salary, offer_sent_at, hired_at, last_message_at, candidate_last_read_at, employer_last_read_at |
...
| `candidate_job_postings` | candidate_id, title, location, work_modes, desired_salary_*, skills, available_from, notice_period_days (smallint, mutually exclusive with available_from: candidate sets one or the other), years_exp (per-role experience), work_eligible (right-to-work confirmation when citizenship differs from the posting location; null when N/A) |
...
- portfolio_quality (0.25) — blends breadth (`min(project_count / 5, 1)`) and completeness (avg per-project: +0.5 file/link, +0.5 skills) at 50/50
- portfolio_skill_coverage (0.25) — `min(distinct_skills / 10, 1)`
- portfolio_feedback (0.10) — avg `portfolio_feedback.rating` (1-5), normalised `(avg - 1) / 4`; defaults to 1.0 until employer feedback arrives
- reputation_score (0.25) — event-driven (ghosted -15, responded +5, completed_match +10), starts at 1.0
- demand_alignment (0.10) — fraction of candidate's portfolio skills that appear in open employer postings, capped at 5 matched skills for full score; defaults to 1.0 when no skills or no openings
- profile_completeness (0.05) — `years_exp_claimed` set + ≥1 `candidate_job_postings` row
...
Two independent timeout rules, both enforced by `GET|POST /api/cron/expire-matches` (`run()` → `{ expired, ghosted_chats }`). Scheduled daily via `vercel.json` (`0 3 * * *`, 03:00 UTC; the Vercel Hobby plan limits crons to once per day, and the 72hr pending-pitch / silent-chat windows tolerate daily granularity. Restore hourly `0 * * * *` if upgraded to Pro). If `CRON_SECRET` is set, the route requires `Authorization: Bearer ${CRON_SECRET}` (Vercel Cron sends this automatically).

- **Pending pitches** (`expirePendingPitches()`): matches expire after 72 hours (`expires_at` default). Accept/decline inserts a `reputation_events` row (`event_type: 'responded'`). Finds `pending` matches where `expires_at < now()` and sets `status: 'ghosted'`. No reputation penalty for the candidate; the pitch is simply closed.
- **Silent chats** (`expireSilentChats()`): an `accepted` match that hasn't reached a hire (`hired_at IS NULL`) is auto-ghosted if neither party sends a `match_messages` row within `CHAT_GHOST_HOURS` (72, `src/lib/utils/constants.ts`) of the last activity — `last_message_at` if a message has been sent, otherwise `responded_at`. The silent party is whoever did **not** send the most recent message (defaults to the employer if no messages were ever sent, since the employer is expected to open the chat). Sets `status: 'ghosted'` and inserts a `reputation_events` row (`event_type: 'ghosted'`, `weight: -10`) against the silent party.
...
- `POST /api/matches/[matchId]/offer` — hire-offer + match-exit state machine layered on an `accepted` match. **Every action requires `matches.status = 'accepted'`** (409 otherwise) and is scoped to the match's participants. Two state fields move: `matches.offer_status` (`null → 'pending' → 'accepted' | 'declined'`) for the hire offer, and `matches.status` (`accepted → 'withdrawn' | 'declined'`) for exiting the match. `offer_status` only ever takes `'pending' | 'accepted' | 'declined'` (migration `0038_merge_reneged_into_declined.sql` collapsed the old `'reneged'` value into `'declined'`; the message `sender_id` and the `reneged` body flag below distinguish the variants). `matches.status = 'withdrawn'` was added in `0039_add_withdrawn_match_status.sql`. Each action also stamps the acting user's own read column via `readColumnUpdate()` and (except `send`/`accept`) notifies the other party via `sendNewMessageNotification()` gated on their `email_notifications`. The actions:
  - `action: "send"` (employer; no offer pending, not yet hired) — sets `offer_status: 'pending'`, `offer_salary`, `offer_sent_at`; inserts a `message_type: 'offer'` message; emails the candidate via `sendHireOfferNotification()`.
  - `action: "accept"` (candidate; `offer_status: 'pending'`) — sets `offer_status: 'accepted'`, `hired_at: now()`; inserts a `message_type: 'offer_accepted'` message; inserts a `reputation_events` row for **each** party (`event_type: 'completed_match'`, `weight: 2`); deletes the matched `candidate_job_postings` row (`matches.candidate_posting_id`, added in `0040_add_candidate_posting_id_to_matches.sql`) so a hired candidate's posting leaves the pool; closes the employer posting when its hired count reaches `max_candidates` and auto-declines all remaining `pending`/`accepted` matches on that posting.
  - `action: "decline"` (candidate; `offer_status: 'pending'`) — rejects the offer, sets `offer_status: 'declined'`; inserts a `message_type: 'offer_declined'` message. The match stays `accepted` so the employer can send a fresh offer.
  - `action: "withdraw"` (employer; `offer_status: 'pending'`) — employer retracts their own pending offer, sets `offer_status: 'declined'`; inserts a `message_type: 'offer_declined'` message (body carries the original `offered_salary`). Match stays `accepted`.
  - `action: "renege"` (candidate; `offer_status: 'accepted'` and `hired_at` set) — candidate backs out **after** accepting: sets `matches.status: 'declined'`, `offer_status: 'declined'`, `hired_at: null`; inserts a `message_type: 'offer_declined'` message with `{ reneged: true }` in the body; **reverses the hire's side effects** — deletes the two `completed_match` `reputation_events` for the match and deletes the `salary_data_points` row sourced from it — and re-triggers the scorer.
  - `action: "withdraw_match"` (candidate; not yet hired) — candidate exits the match entirely: sets `matches.status: 'withdrawn'`, clears `offer_salary`, and downgrades a still-`pending` `offer_status` to `'declined'`; inserts a `reputation_events` row against the employer (`event_type: 'candidate_withdrew'`, `weight: 2`) and re-triggers the scorer.
  - `action: "decline_match"` (employer; not yet hired) — employer ends the match: sets `matches.status: 'declined'`, clears `offer_salary`, and downgrades a still-`pending` `offer_status` to `'declined'`.

  > **`declined` disambiguation in the UI:** several actions land on `matches.status = 'declined'`, so the candidate matches view distinguishes them by shape — a `declined` match with `offer_salary != null && !hired_at` renders as **RENEGED** (candidate backed out of an accepted offer), otherwise as **DECLINED** (pitch/match declined). `withdrawn` renders as **WITHDRAWN**.
- `MatchChat` (`src/components/terminal/MatchChat.tsx`) — polling-based (5s interval; no Realtime, per the known `auth.uid()` RLS limitation above), rendered inside a second `.slideover-panel` (titled `CHAT`) on `/candidate/matches` and the employer posting lobby (`/employer/postings/[postingId]`), alongside the existing `PITCH DETAIL` slide-over. Header strip shows the counterpart's name + score/percentile (employer's view of a candidate) or reputation (candidate's view of an employer), plus the pitch's `offered_salary`. Renders `offer`/`offer_accepted`/`offer_declined` messages as centered system cards and `file` messages as a bordered row with filename, `formatFileSize(file_size)`, and a `DOWNLOAD →` link to the file route above; a `+` button opens a file picker to send attachments. Surfaces the full offer/exit state machine above as buttons, each gated to the eligible party and current state: the employer gets **SEND HIRE OFFER** (when none pending), **WITHDRAW OFFER** (retract a pending offer; chat stays open for a revised one), and **DECLINE MATCH**; the candidate gets accept/decline on a pending offer, **RENEGE OFFER** (after accepting), and **WITHDRAW** from the match. The destructive/irreversible ones (candidate accept/decline, renege, withdraw-from-match) are **multi-layer confirms** — an initial review step then a final step gated behind a required confirmation checkbox (rendered via `Modal`) — so they can't fire on a single accidental tap. An `offer_declined` message renders as "OFFER RENEGED BY CANDIDATE" / "OFFER WITHDRAWN BY EMPLOYER" / "OFFER DECLINED" depending on its `reneged` flag and `sender_id`. Non-`accepted` matches render a "THIS MATCH IS {STATUS} · CHAT CLOSED" state (showing RENEGED for a candidate-reneged `declined` match); a persistent footer reminds both parties of the `CHAT_GHOST_HOURS` reputation penalty for silence.
- **Chat slide-over & unread tracking**: `/candidate/matches` and the employer posting lobby (`/employer/postings/[postingId]`) show a pulsing `.live-dot` on rows with unread activity — `last_message_at` (or `created_at` if no messages) newer than the viewer's `candidate_last_read_at`/`employer_last_read_at` (candidate side treats `NULL` as unread, since `employer_last_read_at` defaults to `now()` at insert but `candidate_last_read_at` starts `NULL`). Both slide-overs share the same `.slideover-panel` position (`right: 0`): `selectedId` state drives `PITCH DETAIL` (opened via row click → `openRow`) and `chatMatchId` state drives `CHAT` (opened via the row's `CHAT →` button or the `PITCH DETAIL` slide-over's `OPEN CHAT →` button → `openChat`, accepted matches only) — each opener clears the other's state so only one slide-over is visible at a time. Both `openRow` and `openChat` call `markRead()`, which stamps the viewer's read column via `POST /api/matches/[matchId]/read` and optimistically mirrors it client-side, clearing the unread dot immediately.
