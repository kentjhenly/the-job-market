...
- **Stripe billing:** `UpgradePanel` (`/employer/feed`) offers two tiers (STARTER HKD 150/mo, PRO HKD 300/mo) and POSTs `{ tier }` to `POST /api/subscription/checkout`, which creates a Stripe Checkout subscription session (price IDs from `STRIPE_PRICE_STARTER`/`STRIPE_PRICE_PRO`, `employer_id`+`tier` stamped into both session and subscription metadata) and returns the redirect `url`. On return, `GET /api/subscription/success?session_id=...` verifies the session and flips the employer to `active`. Ongoing sync is handled by `POST /api/subscription/webhook` (`customer.subscription.created/updated/deleted`, signature-verified via `STRIPE_WEBHOOK_SECRET`): it maps Stripe's status to `active | past_due | canceled`, sets the tier/period-end from the subscription's metadata + first item's `current_period_end`, all keyed by `employer_id` in the subscription metadata. Env: `STRIPE_SECRET_KEY`, `STRIPE_PRICE_STARTER`, `STRIPE_PRICE_PRO`, `STRIPE_WEBHOOK_SECRET` (see `.env.example`). The display prices in `UpgradePanel` are static UI copy; the actual charge comes from the Stripe price objects, so keep the two in step when you change a price in the dashboard. The columns remain manually-settable (e.g. via `/admin/concierge`) as a fallback when Stripe keys aren't configured.
...
- `POST /api/matches/[matchId]/offer` — hire-offer + match-exit state machine layered on an `accepted` match. **Every action requires `matches.status = 'accepted'`** (409 otherwise) and is scoped to the match's participants. Two state fields move: `matches.offer_status` (`null → 'pending' → 'accepted' | 'declined'`) for the hire offer, and `matches.status` (`accepted → 'withdrawn' | 'declined'`) for exiting the match. `offer_status` only ever takes `'pending' | 'accepted' | 'declined'` (migration `0038_merge_reneged_into_declined.sql` collapsed the old `'reneged'` value into `'declined'`; the message `sender_id` and the `reneged` body flag below distinguish the variants). `matches.status = 'withdrawn'` was added in `0039_add_withdrawn_match_status.sql`. Each action also stamps the acting user's own read column via `readColumnUpdate()` and (except `send`/`accept`) notifies the other party via `sendNewMessageNotification()` gated on their `email_notifications`. The actions:
  ...
  - `action: "accept"` (candidate; `offer_status: 'pending'`) — sets `offer_status: 'accepted'`, `hired_at: now()`; inserts a `message_type: 'offer_accepted'` message; inserts a `reputation_events` row for **each** party (`event_type: 'completed_match'`, `weight: 2`); deletes the matched `candidate_job_postings` row (`matches.candidate_posting_id`, added in `0040_add_candidate_posting_id_to_matches.sql`) so a hired candidate's posting leaves the pool; closes the employer posting when its hired count reaches `max_candidates`.
