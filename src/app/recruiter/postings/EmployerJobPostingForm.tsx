...
export function EmployerJobPostingForm({ initial }: EmployerJobPostingFormProps) {
  ...
  async function save(e: React.FormEvent) {
    ...
    const res = await fetch(
      isEditing ? `/api/recruiter-postings/${initial.id}` : "/api/recruiter-postings",
      {
    ...
    if (res.ok) {
      if (isEditing) {
        router.push(`/recruiter/postings/${initial.id}`);
      } else {
        const { id } = await res.json();
        router.push(`/recruiter/postings/${id}`);
      }
    } else {
  ...
  async function confirmDelete() {
    if (!initial) return;
    setDeleting(true);
    const res = await fetch(`/api/recruiter-postings/${initial.id}`, { method: "DELETE" });
    setDeleting(false);
    if (res.ok) router.push("/recruiter/postings");
  }
  ...
  return (
    <div className="view-enter space-y-6">
      <div>
        <Link
          href={isEditing ? `/recruiter/postings/${initial.id}` : "/recruiter/postings"}
      ...
      <form onSubmit={save} className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            type="button"
            variant="ghost"
            onClick={() => router.push(isEditing ? `/recruiter/postings/${initial.id}` : "/recruiter/postings")}
          >
            CANCEL
          </Button>
          {isEditing && (
